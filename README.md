# notroflguru

## О себе
Начинающий backend developer

## Технический стек
![Java](https://img.shields.io/badge/Java-007396.svg?logo=Java&style=flat)
![Spring](https://img.shields.io/badge/Spring-%236DB33F.svg?logo=spring&style=flat&logoColor=white)

![Docker](https://img.shields.io/badge/-Docker-%230db7ed.svg?logo=docker&style=flat&logoColor=white)

![PostgreSQL](https://img.shields.io/badge/PostgreSQL-316192?style=flat&logo=postgresql&logoColor=white)

![Git](https://img.shields.io/badge/Git-%23F05033.svg?logo=git&style=flat&logoColor=white)

## Сейчас изучаю
- JDBC и работу с PostgreSQL
- Принципы чистой архитектуры (SOLID, Dependency Injection)
- Docker для локальной разработки

## Проекты

### Habit Tracker (в разработке)
Консольное приложение для отслеживания привычек.  
**Стек:** Java, PostgreSQL, JDBC, Docker  
- Регистрация и аутентификация пользователей
- CRUD-операции с привычками
- Хранение данных в PostgreSQL, ручная реализация репозиториев

### Mini-Bank
Учебный проект банковской системы.  
**Стек:** Java, коллекции, многопоточность  
- Управление счетами, переводы, транзакции
- Реализация бизнес-логики без фреймворков

## Связь со мной
[![Telegram](https://img.shields.io/badge/Telegram-2CA5E0?style=flat&logo=telegram&logoColor=white)](https://t.me/roflguru)
[![Discord](https://img.shields.io/badge/Discord-%237289DA.svg?logo=discord&style=flat&logoColor=white)]([janp0l](https://discord.gg/ucGKnypa))
